...
fn_dogliotti2015 <- function(p_w){
  ...
  p_w <- ifelse(p_w > 0.16, NA_real_, p_w)
  out <- A_t * (p_w) / 
    (1 - ((p_w)/ C))
  return(out)
}
...
## Import S2 ===========
s2 <- terra::rast("data/processed_RS/S2-composited_red_AMC_2019-01-02_2023-10-28.nc")
names(s2) <- time(s2)
# s2 <- s2[[time(s2) < ymd("2021-01-01")]]
d1 <- as.data.table(s2,xy=T)
