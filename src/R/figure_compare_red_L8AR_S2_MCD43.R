...
list.files("outputs/","_red_")


tmp1 <- arrow::read_parquet("outputs/Landsat8AquaticReflectance_AMC_red.parquet") %>% 
  mutate(sensor = "L8AR")
tmp2 <- arrow::read_parquet("outputs/S2-Level2A_AMC_red.parquet") %>% 
  mutate(sensor = "S2-L2A")
tmp3 <- arrow::read_parquet("outputs/MCD43A4_AMC_red.parquet") %>% 
  mutate(sensor = "MCD43A4")
tmp4 <- arrow::read_parquet("outputs/MOD09GA_AMC_red.parquet") %>% 
...
(p1 <- dat[frac_cover >= 0.25] %>% 
  ...
  ggplot(aes(time_ym, q75,color=sensor))+
  # geom_point() + 
  geom_line(inherit.aes = F,
            aes(time_ym,q75,group=sensor),
            color='black',
            lwd=1.1)+
  geom_line()+
  scale_color_discrete_c4a_cat(palette = 'misc.okabe') + 
  labs(x=NULL,
       y="Monthly Mean: Red 75th perc.",
       color="Product: ") +
  facet_wrap(~dz) +
  theme_linedraw() + 
...
(p2 <- dat[frac_cover >= 0.1] %>% 
  ...
  # .[,.(q75 = mean(q75,na.rm=T)),
  #   by=.(sensor, dz,time_ym)] %>% 
  ggplot(aes(time, q75,color=sensor))+
  geom_point(color='black')+
  geom_point(alpha =0.75, size=1.25) +
  scale_color_discrete_c4a_cat(palette = 'misc.okabe') + 
  labs(x=NULL,
       y="Red reflectance zonal 75th perc.",
...
(p2/p1)
ggsave(filename = "figures/figure_compare_red_L8AR_S2_MCD43.png",
       width = 25,
       height = 20,
       units='cm',
       dpi = 350)
