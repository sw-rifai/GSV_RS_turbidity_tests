...
dat_m <- arrow::read_parquet("outputs/MCD43A4_AMC_red.parquet") %>% 
  ...
  mutate(dog_mean = fn_dogliotti2015(mean))

# table(is.na(dat_m$dog_mean))
quantile(dat_m$dog_mean, c(0.005, 0.995),na.rm=T)
# dat_m$mean %>% hist(100); abline(v=0.15)
# dat_m$dog_mean %>% hist(100)

### mean Weighted MCD43 anomaly =========================
m_ref_mcd <- bam(dog_mean ~ 
                   fzone + 
                   s(doy,fzone,bs=c("fs","cc"),k=6),
                 data=dat_m[dog_mean > 0.01][dog_mean < quantile(dat_m$dog_mean, c(0.995),na.rm=T)],
                 weights = count,
...
dat_m <- dat_m %>% 
  ...
  mutate(dog_mean_anom = dog_mean - dog_u)
...
dat_m2 <- dat_m[is.na(dog_mean)==F][,.(
  val = weighted.mean(dog_mean_anom, count),
  val_sd = sd(dog_mean_anom)),
  by=.(year,month,fzone)][
...
# ggsave(filename = paste0('figures/mcd43a4_weighted_dog-zonalMean-anom_12mo_timeseries_',Sys.Date(),'.png'),
#        width = 30,
#        height = 15,
#        scale = 0.75,
#        units = 'cm',
...
daq <- aq %>% as.data.table(xy=T) %>% 
  ...
  as.data.table()
min_red <- min(daq$red,na.rm=T)
daq[,`:=`(red2 = red+abs(min_red))]
...
daq[,`:=`(dog_anom_z = dog_anom/dog_sd), by=.(x,y,month)]


daq2 <- daq[is.na(dog_anom)==F][,.(
  val = weighted.mean(dog_anom, nobs_zone_time),
  val_z = weighted.mean(dog_anom_z, nobs_zone_time),
  val_sd = sd(dog)),
  by=.(year,month,dz)][
    ,`:=`(d_ym = ymd(paste(year,month,1)))
  ][]
daq2 <- daq2[order(dz,d_ym)][,`:=`(val12 = frollmean(val,n=12,align='right'),
                                   val12_z = frollmean(val_z,n=12,align='right')), 
                                    by = dz]
daq2[,`:=`(dyear = decimal_date(d_ym))]
daq2[,`:=`(product = "AquaOceanColor")]

# daq2 %>% ggplot(aes(dyear,val12_z,color=dz))+geom_line()

### Data: MCD43A4
dmcd <- arrow::read_parquet("outputs/MCD43A4_AMC_red.parquet") %>% 
  mutate(year = year(time),
         month = month(time), 
         doy = yday(time)) %>% 
  mutate(dyear = decimal_date(time)) %>% 
  mutate(dz = factor(dz)) %>% 
  mutate(dog = fn_dogliotti2015(median), 
         product = "MCD43A4")
dmcd[,`:=`(dz = factor(dz,
          ordered = T,
          levels = c("NorthZone_gt10", "NorthZone_lt10", 
...
dmcd[,`:=`(dog_u = mean(dog,na.rm=T),
          dog_sd = mean(dog,na.rm=T)),by=.(dz,month)]
dmcd[,`:=`(dog_anom = dog - dog_u), by=.(dz,month)]
dmcd[,`:=`(dog_anom_z = dog_anom/dog_sd), by=.(dz,month)]

dmcd2 <- dmcd[is.na(dog_anom)==F][,.(
  val = weighted.mean(dog_anom, count),
  val_z = weighted.mean(dog_anom_z, count),
  val_sd = sd(dog_anom)),
  by=.(year,month,dz)][
    ,`:=`(d_ym = ymd(paste(year,month,1)))
  ][]
...
bind_rows(
  daq2[d_ym >=ymd("2002-12-01")][d_ym <= ymd("2023-10-01")] %>% 
    select(dyear,val12,val12_z,dz,product),
  dmcd2[d_ym >=ymd("2002-12-01")][d_ym <= ymd("2023-10-01")] %>% 
    select(dyear,val12,val12_z,dz,product)) %>% 
  select(dyear, dz, product,val12) %>% 
  pivot_wider(names_from = product, 
              values_from = c(val12,product)) %>% 
  group_by(dz) %>% 
  drop_na() %>% 
  summarize(rho = cor(val12_AquaOceanColor,val12_MCD43A4)) %>% 
  knitr::kable()
...
dmcd2[d_ym >=ymd("2002-12-01")][d_ym <= ymd("2023-10-01")] %>% 
  select(dyear,val12,val12_z,dz,product)) %>% 
  ggplot(aes(dyear, val12_z,color=product))+
  geom_hline(aes(yintercept = 0),color='black') +
  geom_smooth(aes(group=product),
    method='lm',se=F,color='black',
              lwd=1.5) + 
  geom_smooth(method='lm',se=F,lwd=1,lty=2) + 
  geom_line(color='black',
            aes(group=product),
            lwd = 1.5) + 
  geom_line() + 
  scale_color_brewer(type='qual',palette = 2) +
  labs(x = NULL, 
       y = "12 mo mean z-score Dogliotti anomaly", 
       color = NULL) + 
  # scale_x_date(date_breaks = "2 years", 
  #              date_labels = "%Y", 
  #              expand = c(0,0)) + 
...
ggsave(filename=
         paste0("figures/figure_compare_AquaOC_MCD43_dogliotti-z_timeseries_",Sys.Date(),".png"), 
       width = 20,
       height = 20,
       scale = 1,
       units = 'cm',
