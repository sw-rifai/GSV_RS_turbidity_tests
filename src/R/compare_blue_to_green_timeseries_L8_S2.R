...
l8_r <- terra::rast("data/processed_RS/LC08_blue_to_green_AMC_2013-08-10_2023-10-09.nc")
system.time(d_l8 <- terra::extract(l8_r, amc3))
names(d_l8) <- c("ID",time(l8_r) %>% as.character())
d_l8 %>% setDT()
d_l8 <- melt(d_l8, id.vars = 1, variable.name='time', value.name = "blue_to_green")
d_l8 <- merge(d_l8, data.table(ID = 1:6, dz = amc3$dz), by='ID')

ss1 <- d_l8[,.(val = mean(blue_to_green,na.rm=T)),by=.(dz,time)]
ss1[,`:=`(time=ymd(time))]
...
# Extract from S2
s2_r <- terra::rast("data/processed_RS/S2-composited_blue_to_green_AMC_2019-01-02_2023-10-28.nc")
system.time(d_s2 <- terra::extract(s2_r, amc3, fun='mean',na.rm=T,bind=T))
d_s2 <- d_s2 %>% 
  as.data.table()
names(d_s2) <- c("dz",as.character(time(s2_r)))

d_s2 <- melt(d_s2, id.vars = 1, variable.name='time', value.name = "blue_to_green")

ss2 <- d_s2[,`:=`(time=ymd(time))]

ss2 %>% 
  ggplot(aes(time,blue_to_green,color=dz))+
  geom_line()


ss3 <- 
  bind_rows(ss1 %>% 
  rename(blue_to_green = val) %>% 
  mutate(sensor = "L8"),
  ss2 %>% 
  mutate(sensor = "S2"))

arrow::write_parquet(ss3,
                     sink="outputs/AMC_zonal_mean_blue_to_green_L8_S2.parquet",
                     compression = 'snappy')
...
ss3 %>% 
  ...
  ggplot(aes(time,blue_to_green,color=sensor))+
  geom_point()+
  ...
  labs(x=NULL,
       y=expression(paste(frac(Blue,Green)))) +
  facet_wrap(~dz,nrow=3) + 
  theme_linedraw() +
  theme(panel.grid.minor = element_blank(), 
        legend.position = 'bottom')

ggsave(filename = "figures/blue_to_green_compare_timeseries_L8_S2_zones.png",
       width = 30,
       height = 15,
       units='cm',
       dpi=350)
