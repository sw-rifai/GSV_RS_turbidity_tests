...
## S2 L2A predict Dogliotti2015 from Acolite 
...
d1 <- c(tmp3,a2) %>% 
  as.data.table(xy=T) %>% 
  set_names(c("x","y","aco_dog","red"))
d1 <- d1 %>% 
  drop_na()

m_dog <- bam(aco_dog ~ s(red,bs='ts',k=6),
                # method='ML',
...
system.time(
d3 <- d2 %>% 
  mutate(dog_pred = predict(m_dog,newdata=.,type='response',discrete=T))
)

d4 <- merge(d3, d1[,.(x,y,aco_dog)], by=c("x","y"))
d4[,`:=`(tsd = as.numeric(time - ymd("2019-08-10")))]

arrow::write_parquet(d4,
                     sink = "data/processed_RS/S2-L2A_dogliotti15_dredge_impact_2019-08-10_2020-03-01.parquet",
                     compression = 'snappy')


d4 <- arrow::read_parquet("data/processed_RS/S2-L2A_dogliotti15_dredge_impact_2019-08-10_2020-03-01.parquet")

ss1 <- d4[,.(val = quantile(dog_pred,0.95, na.rm=T)),by=.(time,tsd)]
ss1 %>%
...
oz_coast <- st_transform(oz_coast, crs = st_crs(a1))
oz_coast <- st_crop(oz_coast, st_bbox(a1))
plot(oz_coast[1])
...
p_out <- d4[y>-34.9][tsd<=48] %>% 
  ...
  labs(title = "Days since peak turbidity",
       x = NULL,
       y= NULL,
       fill= expression(paste(Delta~FNU[Dog.]))) + 
  coord_sf(expand = F, 
           ylim = c(-34.88,-34.59), 
...
ggsave(p_out,
       filename = "figures/figure_dredge_impact_tsd_S2_dogliotti2015.png",
       width = 30,
       height = 30,
       scale = 0.7,
       units='cm',
