...
l8 <- stars::read_stars("data/processed_RS/LC08AR_red_AMC_2013-04-06_2023-10-09.nc", 
                       proxy= T)
...
ss1[n > 3] %>% 
  ...
  labs(x=NULL,y=NULL, 
       ...
       title="Red corr. L8AR & S2") + 
  theme_linedraw()+ 
  theme(panel.grid = element_blank(), 
        panel.background = element_rect(fill='grey'),
        axis.text.x = element_blank())

ggsave("figures/red_correlation_L8AR_S2.png",
       width= 10,
       height = 20,
       units='cm',
       dpi =350)
...
ss1[is.na(rho)==F][sample(.N,10000)] %>% 
  ...
  labs(x = "L8AR, median red reflectance",
       y = "S2 L2A, median red reflenctance",
       title = "48 same day acquisitions",
       subtitle = " (2019-2023)",
...
ggsave("figures/red_long-termMedianComparison_L8AR_S2.png",
       width= 12,
       height = 10,
       units='cm',
       dpi =350)
...
ggsave("figures/red_median_reflectance_L8AR_S2L2A.png",
       width= 20,
       height = 20,
       units='cm',
       dpi =350)
