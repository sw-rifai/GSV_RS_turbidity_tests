...
vec_dates <- fread("data/gee_SAWater_turbidity/DATES_MCD43A4_kd490_mod_daily_2000-02-24_2012-12-31_.csv")
vec_dates2 <- fread("data/gee_SAWater_turbidity/DATES_MCD43A4_kd490_mod_daily_2012-12-31_2023-07-11_.csv")
  
ic <- rast("data/gee_SAWater_turbidity/MCD43A4_kd490_mod_daily_2000-02-24_2012-12-31_.tif")
ic2 <- rast("data/gee_SAWater_turbidity/MCD43A4_kd490_mod_daily_2012-12-31_2023-07-11_.tif")
...
sm <- cdat[order(g,x,y,date)][,
             `:=`(kd_max5d = frollapply(kd, n=5, max, fill=NA, align='right',na.rm=T)),
             by=.(g)]
sm[,`:=`(year=year(date),
         month= month(date))]
...
cdat[date %in% seq(ymd("2016-10-01"),ymd("2016-11-01"),by="1 day")] %>% 
  ...
  theme(axis.ticks.x = element_blank(),
        ...
        legend.position = 'bottom')
ggsave(filename = "figures/MCD43A4_2016-10_kd490.png",
       width = 25,
       height = 20,
       units = 'cm',
       dpi = 350)
