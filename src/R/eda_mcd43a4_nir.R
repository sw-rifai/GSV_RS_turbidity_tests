...
dat <- cbind(tmp_nir, vv_bath %>% select(depth),
             vv_amc %>% select(zone))
...
# cdat[,`:=`(ndvi = (nir-nir)/(nir+nir))]
# cdat[,`:=`(r_nr = nir/(nir+nir))]
# cdat[,`:=`(n_nr = nir/(nir+nir))]
...
sm[date %between% c(ymd("2016-08-01"),ymd("2017-08-01"))][str_detect(g,'_lt5')] %>% 
  ggplot(aes(date, nir95, color = g)) +
  geom_line() + 
...
ggsave(p_out,
       filename = "figures/figure_eda_mcd43a4_nir.png",
       width = 30,
       height = 20,
       units = 'cm',
       dpi = 350)
