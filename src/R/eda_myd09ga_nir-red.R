...
vec_dates <- fread("data/gee_SAWater_turbidity/DATES_MYD09GA_sur_refl_b02_daily_2002-07-04_2012-12-31.csv")
vec_dates2 <- fread("data/gee_SAWater_turbidity/DATES_MYD09GA_sur_refl_b02_daily_2012-12-31_2023-07-01.csv")

ic <- rast("data/gee_SAWater_turbidity/MYD09GA_sur_refl_b02_daily_2002-07-04_2012-12-31.tif")
ic2 <- rast("data/gee_SAWater_turbidity/MYD09GA_sur_refl_b02_daily_2012-12-31_2023-07-01.tif")
...
cdat[date %in% seq(ymd("2016-10-01"),ymd("2016-11-01"),by="1 day")] %>% 
  ...
  labs(fill = "Red Reflectance", 
       title = "MYD09GA - Aqua, Adelaide Metro. Coastal Zones, October 2016", 
       x = NULL,
       y = NULL) +
  facet_wrap(~date, 
             nrow = 3) + 
...
ggsave(filename = "figures/MYD09GA_2016-10_red.png",
       width = 25,
       height = 20,
       units = 'cm',
       dpi = 350)
