...
# Extract from L8AR
ar_r <- terra::rast("data/processed_RS/LC08AR_red_AMC_2013-04-06_2023-10-09.nc")
amc4 <- project(amc3, crs(ar_r))

# plot(ar_r[[1]],col=viridis(100))
# jnk <- ar_r[[1]] %>% as.data.table()
# ar_r[ar_r == 0] <- NA

# terra::project()
# ar_r[[1]] %>% plot(col=viridis(100))

system.time(d_ar <- terra::extract(ar_r, amc4, fun='mean',na.rm=T,bind=T))
d_ar <- d_ar %>% 
  as.data.table()
names(d_ar) <- c("dz",as.character(time(ar_r)))

d_ar <- melt(d_ar, id.vars = 1, variable.name='time', value.name = "red")
d_ar

ss2 <- d_ar[,`:=`(time=ymd(time))]
...
ss3 <- 
  bind_rows(ss1 %>% 
  mutate(sensor = "L8LaSRC"),
  ss2 %>% 
  mutate(sensor = "L8AR"))

arrow::write_parquet(ss3,
                     sink="outputs/AMC_zonal_mean_red_L8AR_L8LaSRC.parquet",
                     compression = 'snappy')
...
ggsave(filename = "figures/red_compare_timeseries_L8LaSRC_L8AR_zones.png",
       width = 30,
       height = 15,
       units='cm',
       dpi=350)
...
# 
# 
# ar <- stars::read_ncdf("data/processed_RS/ar-composited_blue_to_green_AMC_2019-01-02_2023-10-28.nc", 
#                         proxy = F)
# st_crs(ar) <- st_crs(4326)
# 
# 
...
# tmp2 <- ar %>% as.data.table(xy=T)
# 
# tmp3 <- merge(tmp1, tmp2, by=c("x","y","time"))
# 
# tmp4 <- tmp3 %>% drop_na()
# 
# tmp4 %>% select(bg_l8,bg_ar) %>% cor()
# 
# ss1 <- tmp4[,.(rho = cor(bg_l8,bg_ar),
#                l8_p50 = median(bg_l8,na.rm=T),
#                ar_p50 = median(bg_ar,na.rm=T),
#                n = .N),
#             by=.(x,y)]
# 
# ss1[n>3]$rho %>% hist()
...
#        title="Blue/Green corr. from L8 & ar") + 
#   theme_linedraw()+ 
#   theme(panel.grid = element_blank(), 
#         panel.background = element_rect(fill='grey'),
#         axis.text.x = element_blank())
# 
# ggsave("figures/blue_to_green_correlation_L8_ar.png",
#        width= 10,
#        height = 20,
#        units='cm',
#        dpi =350)
...
# 
# tmp4[sample(.N,1000)] %>% 
#   ggplot(aes(bg_l8,bg_ar))+
#   geom_point()+
#   geom_abline(col='red')
# 
# 
# ss1[sample(.N,1000)] %>% 
#   ggplot(aes(ar_p50,rho))+
#   geom_point()
# 
# ss1[sample(.N,1000)] %>% 
