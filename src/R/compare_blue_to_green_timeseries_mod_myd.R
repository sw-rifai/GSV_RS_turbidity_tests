...
# OPTIONS ===================================
var_name <- "blue_to_green" 
sensor1 <- "MOD"
sensor2 <- "MYD"
ylim_range <- c(0.3,2)
  
fp1 <- "data/processed_RS/MOD09GA_blue_to_green_AMC_2000-03-01_2023-10-29.nc"
fp2 <- "data/processed_RS/MYD09GA_blue_to_green_AMC_2002-07-04_2023-10-29.nc"
# ===========================================
...
# Extract from D1 ==============
r1 <- terra::rast(fp1)
system.time(d_r1 <- terra::extract(r1, amc3,fun='mean',na.rm=T,bind=T))
d_r1 <- d_r1 %>% as.data.table()
names(d_r1) <- c("dz",time(r1) %>% as.character())
d_r1 %>% setDT()
ss1 <- melt(d_r1, id.vars = 1, variable.name='time', value.name = var_name) %>% 
  .[,`:=`(time=ymd(time))]

ss1 %>% 
  ggplot(aes(time,eval(as.symbol(var_name)),color=dz))+
  geom_line()

# Extract from D1
r2 <- terra::rast(fp2)
system.time(d_r2 <- terra::extract(r2, amc3,fun='mean',na.rm=T,bind=T))
d_r2 <- d_r2 %>% as.data.table()
names(d_r2) <- c("dz",time(r2) %>% as.character())
d_r2 %>% setDT()
ss2 <- melt(d_r2, id.vars = 1, variable.name='time', value.name = var_name) %>% 
  .[,`:=`(time=ymd(time))]
...
ss3 <- 
  bind_rows(ss1 %>% 
  mutate(sensor = sensor1),
  ss2 %>% 
  mutate(sensor = sensor2))

fp_out <- paste0("AMC_zonal_mean_",var_name, "_",sensor1,"_",sensor2,".parquet")
fp_out
...
ylim_range <- ss3 %>% pull(eval(as.symbol(var_name))) %>% 
  quantile(., c(0.0025,0.9975), na.rm=T)
ss3 %>% 
  arrange(desc(sensor)) %>% 
  filter(time >= ymd("2019-01-01")) %>% 
  ggplot(aes(time,eval(as.symbol(var_name)),color=sensor))+
  geom_point()+
  geom_smooth(se = F,col='black',lwd=1,aes(group=sensor),
              method='bam',
              formula=y~s(x,bs='ad'), 
              method.args=list(discrete=T))+
  geom_smooth(se = F,lwd=0.6,method='bam',
              formula=y~s(x,bs='ad'), 
              method.args=list(discrete=T))+
  coord_cartesian(ylim=ylim_range,expand=F) +
...
fig_name <- paste0(var_name,"_compare_AMC_zonal_mean_",sensor1,"_",sensor2,".png")
ggsave(filename = file.path("figures",fig_name),
       width = 30,
       height = 15,
       units='cm',
       dpi=350)
