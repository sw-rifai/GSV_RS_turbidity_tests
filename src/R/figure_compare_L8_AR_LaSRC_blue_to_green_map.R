...
lasrc <- rast("data/processed_RS/LC08_blue_to_green_AMC_2013-08-10_2023-10-09.nc")
ar <- rast("data/processed_RS/LC08AR_blue_to_green_AMC_2013-04-06_2023-10-09.nc")
...
pal <- viridis::viridis(100,direction = -1)

dir.create("figures/figure_compare_L8_AR_LaSRC_blue_to_green")

fn_make_fig <- function(target_time){
  lasrc1 <- lasrc[[time(lasrc)==target_time]]
  
  ar1 <- ar[[time(ar)==target_time]]
  ar1 <- ar1 %>% project(., crs(lasrc1))
  
  ar1[ar1==0] <- NA
  
  
  fig_fp_out <- paste0("figures/figure_compare_L8_AR_LaSRC_blue_to_green/figure_compare_L8_AR_LaSRC_blue_to_green_",target_time,".png")
  
  lim_lower <- global(ar1,fun=function(x) quantile(x, 1-0.995,na.rm=T)) %>% as.numeric()
  lim_upper <- global(ar1,fun=function(x) quantile(x, 0.995,na.rm=T)) %>% as.numeric()
  ...
  png(filename = fig_fp_out, 
      ...
      res=350)
  par(mfrow=c(1,2))
  plot(ar1,range=c(lim_lower,lim_upper),col=pal,main="L8 Aq. Ref.: Blue/Green", 
       background = "grey40", 
       mar=c(1.5,1,1.5,4))
  lines(amc)
  plot(lasrc1,range=c(lim_lower,lim_upper),col=pal,main="L8 LaSRC: Blue/Green", 
       background = "grey40", 
       mar=c(1.5,1,1.5,4))
  lines(amc)
  dev.off()
