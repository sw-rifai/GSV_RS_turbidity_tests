...
tmp <- list.files("outputs/","AMC_zonal_mean_blue_to_green",full.names =T) %>% 
...
tmp %>% 
  ...
  ggplot(aes(month, blue_to_green,color=sensor))+
...
ggsave(filename = "figures/compare_sensor_blue_to_green_seasonal.png",
       width = 25,
       height = 15,
       units='cm')
